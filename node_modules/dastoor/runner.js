...
Runner.prototype.run = function (node, args) {
    ...
    while(args.length){
        e = args[0];
        if(this._isHelpKey(e)) return this.renderHelp(root, node);
    ...
    if(node){
        ...
        if(hasHelp || !controller) return this.renderHelp(root, node);

        path.push(e);

        this._onNodeStarting(node, path);
...
Runner.prototype.renderHelp = function (root, node) {
    var helpOption = {
